...
export default function LoginPage() {
  ...
  const handleStravaAuth = async () => {
    ...
    if (!token) {
      setError("Please login first to connect Strava");
      return;
    }
    setLoading("strava");
  ...
  const handleGarminAuth = async () => {
    const token = getAuthToken();
    if (!token) {
      setError("Please login first to connect Garmin");
      return;
    }
    setLoading("garmin");
    await new Promise((resolve) => setTimeout(resolve, 500));
      ...
      >
        ...
        <form onSubmit={handleSubmit} className="space-y-5">
          ...
          <motion.button
            ...
            whileHover={loading !== "login" ? { scale: 1.02 } : {}}
            whileTap={loading !== "login" ? { scale: 0.98 } : {}}
