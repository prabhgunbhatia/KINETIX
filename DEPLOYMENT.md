...
5. **Important**: When adding the service, you need to configure it:
   - Click on the service → Settings → Root Directory
   - Set Root Directory to `backend`
   - OR: Add service → Select "Empty Service" → Then set Root Directory to `backend`
6. Railway will automatically detect the `railway.json` or `Procfile` in the backend folder
7. Add environment variables:
   - `DATABASE_URL` (Railway provides PostgreSQL automatically - add PostgreSQL service first)
   - `JWT_SECRET` (generate a random string)
   - `STRAVA_CLIENT_ID`
   - `STRAVA_CLIENT_SECRET`
   - `OPENWEATHER_API_KEY`
   - `STRAVA_REDIRECT_URI` (use Railway's URL: `https://your-app.railway.app/auth/strava/callback`)
